...
# ==========================================
# 1. KONFIGURASI & STYLING (UI PROFESIONAL)
# ==========================================
...
def local_css():
    ...
    <style>
        ...
        /* Styling Tombol Dashboard (Cards) */
        div.stButton > button {
            width: 100%;
            height: auto;
            min-height: 120px;
...
def check_password():
    ...
    with c2:
        st.markdown("""
        <div class="login-box">
            <h1 style="text-align: center; color: #333;">🔐 Login Aplikasi</h1>
            <p style="color: #666;">Masuk untuk mengakses sistem RHK PKH Pro</p>
        </div>
        """, unsafe_allow_html=True)
...
def main_app():
    ...
    # --- SIDEBAR BARU (EXPANDER) ---
    with st.sidebar:
        st.title("⚙️ Pengaturan")
        
        with st.expander("👤 Profil Pendamping", expanded=True):
            nama = st.text_input("Nama Lengkap", u_nama, key="nama_val")
            nip = st.text_input("NIP", u_nip, key="nip_val")
            kpm = st.number_input("Total KPM", value=u_kpm, key="kpm_global_val")
        
        with st.expander("🌍 Wilayah Kerja"):
        ...
        with st.expander("📅 Tanggal Surat"):
            c1, c2 = st.columns(2)
            ...
            st.text_input("Tanggal Full", key="tgl_val")
        
        with st.expander("🖼️ Upload Atribut"):
    ...
    if st.session_state['page'] == 'home':
        st.markdown("## 📂 Dashboard RHK PKH Pro")
        st.markdown("Pilih menu laporan di bawah ini untuk mulai generate dokumen otomatis.")
        ...
        st.markdown("---")
        st.caption("Copyright © 2026 VHS | All Rights Reserved | v51.0")
    ...
    # --- DETAIL PAGE ---
        ...
        if "RHK 3" in current_rhk:
            ...
            ket = st.text_area("Keterangan Tambahan (Opsional):", height=80); fp, nu = render_photo_manager("rhk3")
            ...
            with st.container(border=True):
                ...
                kt = st.text_area("Keterangan (Opsional):", height=80); fp, nu = render_photo_manager("q_rhk")
            ...
            if len(q) > 0:
                ...
                if c2.button("🚀 GENERATE SEMUA", type="primary", use_container_width=True):
                    res = []; prog = st.progress(0); stat = st.empty()
            ...
            kt = st.text_area("Keterangan (Opsional):", height=80); fp, nu = render_photo_manager("biasa")
            if st.button("🚀 Buat Laporan", type="primary", use_container_width=True):
                if nu: 
                    for f in nu: auto_save_photo_local(f, current_rhk, meta['bulan'])
