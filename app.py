...
def main_app():
    ...
    # --- DETAIL PAGE ---
        ...
        if "RHK 3" in current_rhk:
            ...
            # --- TAMBAH TOMBOL DOWNLOAD TEMPLATE EXCEL ---
            ...
            # Perbaiki: Pastikan tombol download excel muncul benar
            ...
            c_dl2.download_button("📥 Template CSV (Alternatif)", template_df.to_csv(index=False).encode('utf-8'), "template.csv", "text/csv")
            
            upl_grad = st.file_uploader("Upload Excel Graduasi", type=['xlsx', 'csv'])
            if upl_grad:
                try: st.session_state['graduasi_raw'] = pd.read_csv(upl_grad) if upl_grad.name.endswith('.csv') else pd.read_excel(upl_grad)
